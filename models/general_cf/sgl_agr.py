...
class SGL_AGR(BaseModel):
    def __init__(self, data_handler):
        super(SGL_AGR, self).__init__(data_handler)
        ...
        # Initialize node and edge droppers, SGL-specific graph enhancement components
    ...
    def _init_dataset_config(self):
        ...
        if self.dataset in configs['model']:
            ...
            self.augmentation = dataset_config.get('augmentation', configs['model'].get('augmentation', 'edge_drop'))
            ...
            # SGL-specific contrastive learning parameters
            ...
            self.augmentation = configs['model'].get('augmentation', 'edge_drop')
    ...
    def forward(self, adj=None, emb_type='cf', keep_rate=1.0, masked_user_embeds=None, masked_item_embeds=None):
        """Forward propagation calculation, generating user and item embedding representations, supporting SGL graph data enhancement"""
        if adj is None:
            adj = self.adj
        ...
        # SGL-specific: node augmentation
        if self.is_training and self.augmentation == 'node_drop':
            embeds = self.node_dropper(embeds, keep_rate)
            
        embeds_list = [embeds]
        
        # SGL-specific: edge augmentation
        if self.is_training and self.augmentation == 'edge_drop':
            adj = self.edge_dropper(adj, keep_rate)
        
        # Multi-layer graph convolutional network propagation - SGL supports random walk augmentation
        for i in range(self.layer_num):
            if self.is_training and self.augmentation == 'random_walk':
                current_adj = self.edge_dropper(adj, keep_rate)
            else:
                current_adj = adj
                
            embeds = self._propagate(current_adj, embeds_list[-1])
    ...
    def hsic_graph(self, users, pos_items):
        ...
        loss_item = hsic(Ki, Kj, items.shape[0])
        
        loss = loss_user + loss_item
        return loss

    def cal_loss(self, batch_data):
        """Calculate overall model loss, integrating multiple learning objectives, including SGL's contrastive learning"""
        ...
        # 3. SGL-specific: Generate two data-augmented embeddings for contrastive learning
        user_graph_embeds1, item_graph_embeds1 = self.forward(
            denoised_adj, 'cf', keep_rate=self.keep_rate,
            masked_user_embeds=masked_user_embeds, masked_item_embeds=masked_item_embeds
        )
        user_graph_embeds2, item_graph_embeds2 = self.forward(
            denoised_adj, 'cf', keep_rate=self.keep_rate,
            masked_user_embeds=masked_user_embeds, masked_item_embeds=masked_item_embeds
        )
        ...
        self.user_emb_old, self.item_emb_old = self.forward(
            self.adj, 'cf', keep_rate=1.0,
            masked_user_embeds=masked_user_embeds, masked_item_embeds=masked_item_embeds
        )
        
        self.user_emb, self.item_emb = self.forward(
            denoised_adj, 'cf', keep_rate=1.0,
            masked_user_embeds=masked_user_embeds, masked_item_embeds=masked_item_embeds
        )
        ...
        # 5. Process prior knowledge provided by LLM
        # 5.1 Process semantic preference knowledge
        ...
        # 5.2 Process structural knowledge
        self.user_emb_str, self.item_emb_str = self.forward(self.aug_adj, 'str', keep_rate=1.0)
        ancstr_embeds, posstr_embeds, negstr_embeds = self._pick_embeds(
            self.user_emb_str, self.item_emb_str, batch_data
        )
        
        # 6. Calculate collaborative filtering main loss
        auc, bpr_loss, reg_loss = self._bpr_loss(batch_data)
        
        llm_auc, llm_bpr_loss, llm_reg_loss = self._bpr_loss(batch_data, 'str')
        
        # 7. SGL-specific: contrastive learning loss calculation
        anc_embeds1, pos_embeds1, neg_embeds1 = self._pick_embeds(user_graph_embeds1, item_graph_embeds1, batch_data)
        anc_embeds2, pos_embeds2, neg_embeds2 = self._pick_embeds(user_graph_embeds2, item_graph_embeds2, batch_data)
        
        cl_loss = cal_infonce_loss(anc_embeds1, anc_embeds2, user_graph_embeds2, self.cl_temperature) + \
                 cal_infonce_loss(pos_embeds1, pos_embeds2, item_graph_embeds2, self.cl_temperature) + \
                 cal_infonce_loss(neg_embeds1, neg_embeds2, item_graph_embeds2, self.cl_temperature)
        cl_loss /= anc_embeds1.shape[0]
        cl_loss *= self.cl_weight
        
        # 8. Preference knowledge contrastive learning loss
        ...
        # 9. Structural knowledge contrastive learning loss
        ...
        # 10. Information bottleneck regularization, reducing redundant information
        users, pos_items, neg_items = batch_data
        ib_loss = self.hsic_graph(users, pos_items) * self.beta
        
        # 11. Total loss calculation, integrating SGL contrastive learning loss
        loss_cf = bpr_loss + reg_loss
        loss_llm = (prf_loss + str_loss) * self.alpha
        loss = loss_cf + loss_llm + ib_loss + cl_loss  # Add SGL contrastive learning loss
        ...
        losses = {
            ...
            'str_loss': str_loss,
            'ib_loss': ib_loss,
            'cl_loss': cl_loss  # SGL-specific contrastive learning loss
        }
    ...
    def full_predict(self, batch_data):
        ...
        denoised_adj = self.learn_graph_structure(self.adj, configs.get('cf_index', None))
        user_embeds, item_embeds = self.forward(denoised_adj, 'cf', keep_rate=1.0)
